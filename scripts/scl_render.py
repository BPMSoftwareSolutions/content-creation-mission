...
def lower(g, scenario_id):
    ...
    data = dict(contractVersion='infographic-projection.v1',grammarVersion='sidefx-infographic-grammar.v1',
        id=g.id,title=scenario.label[:100],subtitle='SCL 0.1 / '+('Target design' if g.status=='DRAFT' else 'Declared source boundary'),
        ...
        scenarios=[scenario.model_dump()],nodes=[n.model_dump() for n in g.nodes if n.id in ids],junctions=[n.model_dump() for n in g.junctions if n.id in ids],
...
def main():
    p = argparse.ArgumentParser(description=__doc__); p.add_argument('input'); p.add_argument('--scenario'); p.add_argument('--enhanced',action='store_true'); p.add_argument('--output',required=True); a=p.parse_args()
